...
- Make a button to upload a local .txt file and with the help of AJAX will have a functionality of taking only .txt file.
...
- I have a function(uniqueWords) that takes line as input and use Hash table to store each unique words from the input. I will make a button on the website where a customer can upload multiple files first. Build another function that takes multiple files and parse each file's text into one text. With the already built function I wouldn't need to modify the logic.

Question 5: How would you test the program from question 1?

- I would need to implement a way to parse a txt file utilizing XMLHttpRequest or other modules to strings. Since each functions were tested with custom strings and they work individually as intended.
